# tests/rhosocial/activerecord_oracle_test/feature/backend/test_ddl_source_mixin.py
...
VALUE_OPTIONS = (
    OracleColumnOptions(invisible=True),
    OracleColumnOptions(invisible=False),
)
VALUE_OPTIONS_MARKER = UseColumnOptions(*VALUE_OPTIONS)
SINGLE_OPTION = OracleColumnOptions(invisible=True)
...
def test_field_declarations_are_collected_in_declaration_order(model):
    ...
    assert options[0].invisible is True
    assert options[1].invisible is False
...
def test_collected_backend_data_type_renders_with_dialect():
    ...
    assert rendered == ('"VALUE_COL" VARCHAR2(64)', ())
...
def test_table_declarations_are_returned_unchanged(model):
    ...
    assert model.table_options().or_replace is True
