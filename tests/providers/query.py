...
class QueryProviderBase:
    # Cross-schema fixture namespaces: testsuite SCHEMA_A / SCHEMA_B map to
    # Oracle user schemas (uppercased unquoted identifiers).
    _SCHEMA_USERS = ("AR_CRM", "AR_SHOP")
    _SCHEMA_USER_PASSWORD = "Rh0social#2026"

    def __init__(self):
        self._scenario_db_files = {}
        self._schema_users_provisioned = False
    ...
    def _drop_user_block(self, user: str) -> str:
        return (
    ...
    def _cross_schema_statements(self) -> List[str]:
        statements = []
        for user in self._SCHEMA_USERS:
            statements.append(self._drop_user_block(user))
        for user in self._SCHEMA_USERS:
            statements.append(f'CREATE USER {user} IDENTIFIED BY "{self._SCHEMA_USER_PASSWORD}"')
            statements.append(f"GRANT UNLIMITED TABLESPACE TO {user}")
        statements.append(
    ...
    def _provision_cross_schema_sync(self, backend_instance) -> None:
        ...
        self._schema_users_provisioned = True

    def _mixed_schema_statements(self) -> List[str]:
        """Provision an identical orders table inside the AR_CRM user schema."""
        statements = []
        for user in self._SCHEMA_USERS:
            statements.append(self._drop_user_block(user))
        for user in self._SCHEMA_USERS:
        ...
        statements.append(
            ...
            "id NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "
            "user_id NUMBER NOT NULL, order_number VARCHAR2(255) NOT NULL, "
    ...
    def _drop_cross_schema_users_sync(self, backend_instance) -> None:
        options = self._ddl_options()
        for user in self._SCHEMA_USERS:
            try:
                backend_instance.execute(self._drop_user_block(user), options=options)
            except Exception:
                pass
        self._schema_users_provisioned = False

    async def _drop_cross_schema_users_async(self, backend_instance) -> None:
        options = self._ddl_options()
        for user in self._SCHEMA_USERS:
            try:
                await backend_instance.execute(self._drop_user_block(user), options=options)
            except Exception:
                pass
        self._schema_users_provisioned = False
...
class QuerySyncProvider(QueryProviderBase, IQuerySyncProvider, WorkerTestProtocol):
    ...
    def setup_schema_fixtures(self, scenario_name: str):
        """Two models bound to distinct user schemas (see testsuite schema_models)."""
    ...
    def setup_mixed_schema_fixtures(self, scenario_name: str):
        ...
        backend_class, config = get_scenario(scenario_name)
        Order.__connection_config__ = config
        Order.__backend_class__ = backend_class
        Order.__backend__ = shared_backend
        self._reset_schema(shared_backend, "orders")
        ...
        return user_model, Order, MixedSchemaOrder
            ...
            try:
                for table_name in self._table_cleanup_order():
                    try:
                        ...
                        pass
                if self._schema_users_provisioned:
                    self._drop_cross_schema_users_sync(backend_instance)
            finally:
                try:
...
class QueryAsyncProvider(QueryProviderBase, IQueryAsyncProvider):
    ...
    async def setup_schema_fixtures(self, scenario_name: str):
        ...
        _, config = get_scenario(scenario_name)
        await AsyncSchemaCustomer.configure(config, AsyncOracleBackend)
        AsyncSchemaOrder.__connection_config__ = config
        AsyncSchemaOrder.__backend_class__ = AsyncOracleBackend
    ...
    async def setup_mixed_schema_fixtures(self, scenario_name: str):
        ...
        _, config = get_scenario(scenario_name)
        user_model = await self._setup_model_async(AsyncUser, scenario_name, "users")
        shared_backend = user_model.__backend__
        if shared_backend not in self._active_async_backends:
            self._active_async_backends.append(shared_backend)
        order_model = await self._setup_model_async(AsyncOrder, scenario_name, "orders", shared_backend)

        AsyncMixedSchemaOrder.__connection_config__ = config
        AsyncMixedSchemaOrder.__backend_class__ = AsyncOracleBackend
            ...
            try:
                for table_name in self._table_cleanup_order():
                    try:
                        ...
                        pass
                if self._schema_users_provisioned:
                    await self._drop_cross_schema_users_async(backend_instance)
            finally:
                try:
