# src/rhosocial/activerecord/backend/impl/oracle/cli/named_migration.py
"""named-migration subcommand — Oracle adapter for the shared CLI helper."""
...
def create_parser(subparsers):
    ...
    nm_epilog = """Examples:
  # Apply a migration
  %(prog)s myapp.migrations.v001.CreateUsersTable --host localhost --service XEPDB1 --direction up

  # Rollback
  %(prog)s myapp.migrations.v001.CreateUsersTable --host localhost --service XEPDB1 --direction down

  # Apply with record store
  %(prog)s myapp.migrations.v001.CreateUsersTable --service XEPDB1 --record-store ./migrations.json

  # Dry-run
  %(prog)s myapp.migrations.v001.CreateUsersTable --service XEPDB1 --direction up --dry-run

  # Apply asynchronously
  %(prog)s myapp.migrations.v001.CreateUsersTable --service XEPDB1 --direction up --async
